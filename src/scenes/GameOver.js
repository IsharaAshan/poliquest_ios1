...
export default class GameOver extends Phaser.Scene {
    ...
    styleHtmlInput(input, isNickname) {
        ...
        // Base styling with improved visibility
        input.style.color = '#FFFFFF';
        input.style.backgroundColor = 'rgba(0, 0, 0, 0.8)'; // Increased opacity
        input.style.border = '2px solid rgba(255, 255, 255, 0.8)'; // More visible border
        ...
        // Add touch-specific attributes for better mobile handling
        input.setAttribute('autocomplete', 'off');
        input.setAttribute('autocorrect', 'off');
        input.setAttribute('autocapitalize', 'off');
        input.setAttribute('spellcheck', 'false');

        // Focus handling specific for iOS
        input.addEventListener('focusin', () => {
            ...
            // Add slight delay before scrolling into view on iOS
            setTimeout(() => {
        ...
        input.addEventListener('blur', () => {
            input.style.border = '2px solid rgba(255, 255, 255, 0.8)';
        });
    }
    ...
    positionHtmlInput(htmlInput, isNickname) {
        ...
        // Adjust positioning for iOS devices
        const yOffset = isIOS ? 
            (isNickname ? -160 : -100) : // iOS offsets
            (isNickname ? -180 : -120);  // Normal offsets
            
        const yPos = containerY + (yOffset * scaleY * this.log_container.scale);

        // Different widths for different fields
        const width = isNickname ? 280 : 340;
        
        // Set position and dimensions with iOS-specific adjustments
        ...
        const fontSize = isIOS ? 16 : (isNickname ? 20 : 18);
        ...
        htmlInput.style.webkitTransform = 'translateZ(0)';
    }
    ...
    updateHtmlInputPositions() {
        ...
        if (this.nicknameInput && this.bscInput && containerVisible) {
            ...
            // Adjust vertical positioning based on orientation and DECREASE THE GAP here
            // Decrease the gap by moving nickname field lower and BSC field higher
            const nickNameYOffset = isPortrait ? -160 : -180; // Back to original values (from -220)
            const bscYOffset = isPortrait ? -90 : -130;       // Move up from -80 to -130
